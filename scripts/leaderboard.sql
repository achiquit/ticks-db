SELECT partners.fname || ' ' || substr(partners.lname) AS 'Partner', COUNT(DISTINCT ticks.date) AS 'Days', SUM(ticks.pitches) AS 'Pitches', printf('%,d', SUM(ticks.height)) AS 'Height(ft)', COUNT(DISTINCT climbs.area) AS 'Areas'
FROM
    ticks
