SELECT tick_date AS 'Date', climb_name AS 'Climb', grade AS 'Grade', type AS 'Type', pitches AS 'Pitches', height as 'Height(ft)', style AS 'Style', success AS 'Success', group_concat(concat, ', ') AS 'Partner(s)', notes AS 'Notes'
FROM (
    SELECT ticks.id AS tick_id, ticks.date AS tick_date, climbs.name AS climb_name, climbs.grade AS grade, climbs.type AS type, ticks.pitches AS pitches, ticks.height AS height, ticks.style AS style, ticks.success AS success, 
    CASE 
